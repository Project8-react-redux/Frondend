...
const Matches = () => {
    ...
    const filterdLeagues = allLeagues.filter((e) => e.country_id == currentCountryId);
    //=============================================================================================================================
    ...
    function handeLOnChange(e) {
        setDateAndLeague(prevs => ({ ...prevs, [e.target.name]: e.target.value }))
    ...
    const previouMatches = useSelector((state) => state.matches.previousMatches) || []; // get the previous matches to send it to the previous component
    ...
    return (
        <>
            <section className="match-section set-bg" data-setbg="img/match/match-bg.jpg">
                ...
                {previouMatches !== null ? <h2 className='text-light mb-4'>select a <span id='colorRed'>city</span> and a <span id='colorRed'>league</span> to see the results</h2> : ''}
                    <div className='select-div d-flex align-items-center flex-wrap gap-2 '>

                        <div className="dropdown">
                            <button onClick={() => myFunction()} className="dropbtn">Select City</button>
                            <div id="myDropdown" className="pointer dropdown-content">
                                <input className='pointer' type="text" placeholder="Search.." id="myInput" onChange={element => dispatch(saveChange(element.target.value))} />
                                {allCountries.slice(2, 12)?.map((element) => {
                                    return (
                                        <div className='search'>
                                            <p onClick={() => changeIdState(element.country_id)}><img width={'100px'} height={'50px'} src={element.country_logo} alt="" />{element.country_name}</p>
                                        </div>
                                    )
                                })
                                }
                        ...
                        <div className='col-md-4 col'>
                            <select className='col-md-8' id='leagues' name="leagueId" onChange={handeLOnChange}>
